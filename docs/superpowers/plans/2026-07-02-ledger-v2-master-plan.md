...
2. **Plan each phase just-in-time — with a strong-model planning subagent.** Phase 1 already has a detailed plan (`2026-07-02-phase-1-foundation.md`). For every later phase, BEFORE executing it, **dispatch a fresh subagent to write the plan** (via the Agent tool), with the model set to **`opus` (Opus 4.8)** or **`sonnet` (Sonnet 5, medium reasoning)** — NOT your own execution model. Instruct that subagent to invoke **superpowers:writing-plans** and produce `docs/superpowers/plans/2026-07-02-phase-<N>-<name>.md` from the matching spec section, planning against the *current* codebase (it will have changed since v1.0). Give the subagent: the spec path, this master plan's Global Constraints, and the phase's spec section. When it returns the saved plan path, review it briefly, then execute it. Rationale: planning quality gates everything downstream, so spend the stronger model on the plan; keep the lighter execution model for the task-by-task subagents in step 3.
