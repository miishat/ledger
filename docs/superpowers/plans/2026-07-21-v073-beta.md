...
- Produces: version `0.7.3-beta`, changelog entry.

**What's New note:** the What's New modal renders `CHANGELOG.md` directly (imported `?raw` and split by `##` headings in `WhatsNewModal.tsx`). There is NO separate content array in `src/utils/whatsNew.ts` to edit; that file only holds the `shouldShowWhatsNew` version-comparison helper, and its test asserts only that helper. Do not add a phantom entry there. Adding the CHANGELOG entry IS the What's New update.

- [ ] **Step 1: Read the existing conventions**

Open `CHANGELOG.md` and note the exact shape of the most recent entry (`0.7.2-beta`). Match it precisely: `## [0.7.3-beta] - 2026-07-21`, then `### Added` / `### Changed` / `### Fixed` subsections with `- ` bullets. Leave the `## [Unreleased]` heading in place above the new entry.
...
Run: `npx vitest run && npx tsc -b && npx eslint .`
Expected: vitest and tsc clean. eslint has a large pre-existing baseline of project-wide issues unrelated to this release; confirm only that the files you changed introduce no NEW eslint errors.
