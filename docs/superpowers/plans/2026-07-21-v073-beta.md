...
In `src/components/investments/HoldingRow.tsx` and `HoldingCard.tsx`, make the currency text a select. Replace the `{holding.currency ?? 'Set currency'}` expression with:

```tsx
          <ThemedSelect
            value={holding.currency ?? ''}
            onChange={(v) => setHoldingCurrency(holding.id, v ? (v as Currency) : null)}
            aria-label={`Currency for ${holding.ticker}`}
            className="text-[11px]"
            options={[
...
Keep the `unconverted` and quote-source spans next to it. Verify `ThemedSelect` accepts `aria-label` and `className`; if it does not, pass the label through whatever prop it exposes and adjust the Task 11 test selector to match.
...
```bash
git add -A src/components/investments
